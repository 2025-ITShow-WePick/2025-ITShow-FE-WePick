...
// API_BASE_URL 정의 - 수정된 부분
const API_BASE_URL = import.meta.env.PROD
  ? "" // 프로덕션에서는 상대경로 (Vercel rewrites 사용)
  : "/api"; // 개발환경에서는 프록시 사용
  ...
  try {
    ...
    if (error.response) {
      ...
      if (error.response.status === 413) {
        throw new Error(
          "이미지 파일 크기가 너무 큽니다. 더 작은 파일을 선택해주세요."
        );
      } else if (error.response.status === 415) {
        throw new Error(
          "지원하지 않는 이미지 형식입니다. JPG, PNG, GIF 파일만 업로드 가능합니다."
        );
      } else if (error.response.status === 400) {
        throw new Error(
          error.response.data?.message || "잘못된 이미지 파일입니다."
        );
      } else if (error.response.status === 500) {
  ...
  try {
    ...
    if (error.response) {
      ...
      if (error.response.status === 400) {
        throw new Error(
          error.response.data?.message || "게시물 데이터가 올바르지 않습니다."
        );
